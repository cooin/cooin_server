...
module.exports.queryForCurrentUser = {
  ...
  handler: function (request, reply) {
    ...
    RiskKLine.findAll({attributes: config.attributes.riskKLine.list, where: criteria}).then(data => {
      reply(data);
    }).catch(err => {
...
module.exports.queryFollowForUser = {
  ...
  handler: function (request, reply) {
    ...
    Master_register.findOne({where: {activecode: params.userId}}).then(data => {
      ...
      return RiskKLine.findAll({attributes: config.attributes.riskKLine.list, where: criteria});
    }).then(data => {
      reply(data);
    }).catch(err => {
      if (!err.isBoom && err instanceof Error) err = Boom.wrap(err, 400);
