...
exports.register = (server, options, next) => {
  ...
  internal.queryTrade = (coinType, sourceId) => {
    return new Promise((resolve, reject) => {
      //查询参数
      const query = {market: `${coinType}cny`};
      if (sourceId) query.from = sourceId;
      request({
        url: `https://yunbi.com//api/v2/trades.json?${qs.stringify(query)}`,
        method: "GET",
        json: true
    ...
    return Trade.findOne({
      ...
      data.reverse().forEach(item => {
        docs.push({
          source: source,
          sourceId: item.id,
          type: item.side == 'down' ? 'buy' : 'sell',
          coinType: coinType,
          amount: item.volume,
          price: item.price,
          createdAt: new Date(parseInt(item.at) * 1000)
        });
      });
  ...
  internal.queryDepth = (coinType) => {
    return new Promise((resolve, reject) => {
      request({
        url: `https://yunbi.com//api/v2/depth.json?market=${coinType}cny`,
        method: "GET",
        json: true
      }, function (err, httpResponse, body) {
        if (err) reject(err);
